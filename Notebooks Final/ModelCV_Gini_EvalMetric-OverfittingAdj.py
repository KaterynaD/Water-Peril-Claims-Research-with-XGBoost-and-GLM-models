...
def cv_misc_callback(oof_train_scores:list, oof_valid_scores:list, best_models:list, NeedModelsFlg='N', maximize=True):
    ...
    def callback(env):
        ...
        train_score_mean=np.mean(list(map(float,folds_train_scores)))
        train_score_std=np.std(list(map(float,folds_train_scores)), ddof=1)
        valid_score_mean=np.mean(list(map(float,folds_valid_scores)))
        valid_score_std=np.std(list(map(float,folds_valid_scores)), ddof=1)
        obj = valid_score_mean - ((train_score_mean - valid_score_mean)**4)*1000000
        n = len(oof_train_scores) % 10
        if n==0:
            print('[%s]\ttrain-gini:%.5f+%.5f\tvalid-gini:%.5f+%.5f\tobjective:%.5f'%(len(oof_train_scores),train_score_mean,train_score_std,valid_score_mean,valid_score_std,obj))
...
def _xgb_cv(params, dtrain,  num_boost_round, nfold, early_stopping_rounds, model_dir, output_data_dir, GetFIFlg,GetTestScoreFlg,GetTestPredFlg,is_master):
    ...
    cv_results=xgb.cv(params, 
                      ...
                      seed=42,
                      callbacks=[cv_misc_callback(oof_train_scores, oof_valid_scores,best_models,NeedModelsFlg,True)]
                     )
